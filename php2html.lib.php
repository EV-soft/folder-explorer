<?   $DocFileLib='../php2html.lib.php';    $DocVers='1.2.0';    $DocRev1='2022-10-07';     $DocIni='evs';  $ModulNo=0; ## File informative only
...
function htm_Caption(# $labl='',$icon='',$hint='',$algn='',$styl='color:#550000; font-weight:600; font-size: 13px;');
    ...
    $styl='color:#550000; font-weight:600; font-size: 13px;'   # string: Default Caption text style
) {
...
function htm_TextDiv(# $body,$algn='left',$marg='8px',$styl='box-shadow: 3px 3px 6px 0px #ccc; padding: 5px; border: solid 1px lightgray; ',$attr='background-color: white; ');
    ...
    $styl='box-shadow: 3px 3px 6px 0px #ccc; padding: 5px; border: solid 1px lightgray; white-space: nowrap; ', # string: Other style
    $attr='background-color: white; '                                                                           # string: Other style 
) {
...
function htm_Panel_0( # $capt= '',$icon= '',$hint= '',$form= '',$acti= '',$clas= 'panelWmax',$wdth= '',$styl= 'background-color: white;',$attr= '',$show = true,$head = '' /* ,$where='Undefined',$BookMark='' */ );
...
/* 
JS:
$("#input_hidden_field").val(JSON.stringify(arrData));  //store array
$.post("test.php", {data: arrData}).always(function() { window.location = "test.php"; });    
var value = $("#input_hidden_field").val();             //retrieve array
value = JSON.parse(value);

HTML:
<input type="hidden" id="input_hidden_field" name="input_hidden_field" value="'.$arrData.'">
PHP:
$arrData = json_decode($_POST["input_hidden_field"], true);
*/
...
## Function:
    ...
    $result.= '  <button class="acceptbutt buttstyl" '.$form.' type= "'.$type.'" name="btn_'.$midn.$name.'" id="btn_'.$midn.$name.'" '.$attr.
              '    style="min-height: 24px; padding-right: 6px; margin-bottom: 6px; '.$wdth. $colors.'" onclick=\''.$acti.'\' accesskey="'.$akey.'"> '.
                    $iconClass .$Label.
              '  </button>';
...
function htm_ActionButt(# $labl,$icon='',$hint='',$type='button',$name='',$form='',$acti='',$attr='',$rtrn=true)
    ...
    $attr='',       # string: 
    $rtrn=true      # bool:   return string else echo string
    ) 
{ global $ButtName;
    $ButtName= $name;
    if ($hint>'') {
...
function htm_MultistateButt(# string: # $name='ROWyCOLx', $valu='', $acti=true, $styl='padding:1px;') 
...
function htm_Tab(# $labl='',$body='',$name='',$styl='text-align: left; box-shadow: 3px 3px 6px 0px #ccc; padding: 5px; background-color: white;',$bclr='white')
    ...
    $body='',                                                                                           # string: 
    $name='',                                                                                           # string: 
    $styl='text-align: left; box-shadow: 3px 3px 6px 0px #ccc; padding: 5px; background-color: white;', # string: 
    $bclr='white'                                                                                       # string: 
    ) 
{   $GLOBALS['TabLabl'].= '
...
if (!function_exists('lang')) {
    function lang($txt)     # lang() is used to translate all hard-coded user interface
    {   global  $natindx, $engindx, $allLang, $lang;
        $txt= trim($txt,"@");
        if      (isset($allLang[$natindx]['translation'][$txt]))    return /* sys_enc */($allLang[$natindx]['translation'][$txt]); // .' ('.$lang.')');   // National
        else if (isset($allLang[$engindx]['translation'][$txt]))    return /* sys_enc */($allLang[$engindx]['translation'][$txt]);                        // Translated English
        else                                                        return trim($txt,"@");  // Default English, if not in translate table           // Native (english)
    }
}
...
KeyWords for syntax highlighting:
 labl capt body plho icon hint type name valu form subm acti clas wdth algn marg styl attr font colr fclr bclr llgn link targ akey kind 
 echo ftop unit disa rows step bord rtrn titl info inis gbl_imag gbl_bord simu clck vhgh stck cntx html butt mess tplc tsty head elem
...
Systemets integrerede fordele:                                  Integrated benefits of the system:
Avanceret oversættelses system                                  Advanced translation system
Blokorienteret giver kompakt kode                               Block-oriented provides compact code
Mulighed for Font-awesome Ikoner overalt                        Possibility of Font-awesome Icons anywhere
Tooltip ved alle html-elementer                                 Tooltip for all html elements
Kompakt layout med sammenklappelige paneler                     Compact layout with collapsible panels
Mulighed for bruger definerede systemblokke                     Option for user defined system blocks
Avancerede tabeller baseret på Mottie Tablesorter-system        Advanced tables based on Mottie Table Sorter system
Valgfri benyttelse af biblioteker lokalt eller på Web (CDN)     Optional use of libraries locally or on the Web (CDN)
